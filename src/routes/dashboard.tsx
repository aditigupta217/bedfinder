...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Live overview of hospital bed availability across the city.",
      },
    ],
  }),
  component: Dashboard,
...
function Dashboard() {
  ...
  const icuTone: "green" | "amber" | "red" =
    icuPctFree > 20 ? "green" : icuPctFree >= 10 ? "amber" : "red";
  ...
  const sorted = useMemo(
    () =>
      [...hospitals].sort((a, b) =>
        sortDesc ? b.icu_available - a.icu_available : a.icu_available - b.icu_available,
      ),
    [hospitals, sortDesc],
  );
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:py-10">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <h1 className="font-display font-bold text-3xl sm:text-4xl text-[#0A1628]">
              City Health Dashboard
            </h1>
            <p className="text-muted-foreground mt-2">
              Live overview of bed availability across government hospitals
            </p>
          </div>
        ...
        <div className="mt-6 bg-white border border-[#E8ECF2] rounded-xl p-5 sm:p-6">
          <div className="flex items-baseline justify-between">
            <h3 className="font-display font-semibold text-[#0A1628]">
              City is at {icuPctUsed.toFixed(0)}% ICU capacity
            </h3>
            <span className="text-xs text-muted-foreground">
              {totalIcuCap - totalIcu} used / {totalIcuCap} ICU beds citywide
            </span>
          </div>
          <div className="mt-3 h-3 rounded-full bg-[#F4F6F9] overflow-hidden">
            <div
              className="h-full rounded-full transition-all"
              style={{ width: `${icuPctUsed}%`, backgroundColor: meterTone }}
            />
          </div>
        </div>
                ...
                {sorted.map((h) => {
                  ...
                  return (
                    <tr key={h.id} className="border-t border-[#E8ECF2] hover:bg-[#F4F6F9]/50">
                      ...
                      <td className="px-4 py-3">
                        {h.general_available}/{h.general_total}
                      </td>
                      <td className="px-4 py-3">
                        {h.emergency_available}/{h.emergency_total}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">
                        {minutesAgo(h.last_updated)}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${c}`}
                        >
                          {label}
                        </span>
                      </td>
        ...
        <div className="mt-6 bg-white border border-[#E8ECF2] rounded-xl p-5 sm:p-6">
          <h3 className="font-display font-semibold text-[#0A1628]">
            Hospital Availability Heatmap
          </h3>
          <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {hospitals.map((h) => {
              const s = statusFromIcu(h.icu_available);
              const bg =
                s === "green" ? "bg-[#16A34A]" : s === "amber" ? "bg-[#D97706]" : "bg-[#E8341C]";
              return (
